...
function updateTime() {
    ...
    sElement.style.transform = `rotate(${AngSecond}deg)`; //Trocamos a rotação do ponteiro de segundos a cada vez que a função de atualização da hora for rodada, passando dentro do template string o valor exato do ângulo do segundo.
    mElement.style.transform = `rotate(${AngMinute}deg)`; //Trocamos a rotação do ponteiro dos minutos baseado na variável AngMinute
    hElement.style.transform = `rotate(${AngHour}deg)`; //Trocamos a rotação do ponteiro das horas
}
